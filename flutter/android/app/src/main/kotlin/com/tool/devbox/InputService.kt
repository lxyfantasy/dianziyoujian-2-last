...
const val WHEEL_STEP = 160
const val WHEEL_DURATION = 100L
const val LONG_TAP_DELAY = 1000L
const val DOUBLE_CLICK_INTERVAL = 300L // 双击判定间隔（300ms）
...
class InputService : AccessibilityService() {
	...
	// 右键相关变量 - 重构核心
    private var isRightClickFirst = false // 标记是否是右键第一次点击
    private var rightClickFirstTime = 0L // 右键第一次点击的时间戳
    private var isWaitingRightLongPress = false // 右键长按标记
    // 改用 Handler 管理延迟任务（替代 TimerTask，更可控）
    private val mainHandler = Handler(Looper.getMainLooper())
    private var rightSingleClickRunnable: Runnable? = null // 右键单击延迟执行的Runnable
    private var rightLongPressRunnable: Runnable? = null // 右键长按延迟执行的Runnable
    ...
    fun onMouseInput(mask: Int, _x: Int, _y: Int) {
        ...
        if (mask == RIGHT_DOWN) {
            ...
            // 1. 判定是否为双击：两次点击间隔小于300ms
            if (isRightClickFirst && (currentTime - rightClickFirstTime) < DOUBLE_CLICK_INTERVAL) {
                Log.d(logTag, "右键双击触发 - 返回桌面")
                
                // 核心修复：立即移除所有待执行的右键任务（单击+长按）
                removeAllRightClickRunnables()
            ...
            // 2. 不是双击，处理第一次右键按下
            Log.d(logTag, "右键第一次按下 - 记录状态")
            isRightClickFirst = true
            rightClickFirstTime = currentTime
            isWaitingRightLongPress = true

            // 3. 注册右键长按任务（延迟longPressDuration执行）
            rightLongPressRunnable = Runnable {
                if (isWaitingRightLongPress) {
                    Log.d(logTag, "右键长按触发 - 显示任务栏")
            ...
            mainHandler.postDelayed(rightLongPressRunnable!!, longPressDuration)
            return
        }
        
        // ==================== 重构后的右键抬起逻辑 ====================
        if (mask == RIGHT_UP) {
            Log.d(logTag, "右键抬起 - 取消长按等待")
            isWaitingRightLongPress = false
            
            // 1. 取消长按任务（如果还没执行）
            if (rightLongPressRunnable != null) {
                mainHandler.removeCallbacks(rightLongPressRunnable!!)
                rightLongPressRunnable = null
            }
            
            // 2. 如果是第一次点击，注册单击延迟任务（等待双击判定）
            if (isRightClickFirst) {
                // 先移除已有单击任务（避免重复）
                if (rightSingleClickRunnable != null) {
                    mainHandler.removeCallbacks(rightSingleClickRunnable!!)
                }
                
                // 3. 延迟DOUBLE_CLICK_INTERVAL执行单击（等待双击判定）
                rightSingleClickRunnable = Runnable {
                    Log.d(logTag, "右键单击触发 - 返回操作")
                    performGlobalAction(GLOBAL_ACTION_BACK)
                    resetRightClickState()
                }
                mainHandler.postDelayed(rightSingleClickRunnable!!, DOUBLE_CLICK_INTERVAL)
            }
            return
        }
        ...
        // long WHEEL_BUTTON_DOWN -> 模拟电源键长按（调出重启/关机界面）
        if (mask == WHEEL_BUTTON_DOWN) {
            timer.purge()
            recentActionTask = object : TimerTask() {
                override fun run() {
                    // 切换到主线程执行无障碍操作（RustDesk 跨线程需主线程）
                    Handler(Looper.getMainLooper()).post {
                        try {
                            // 优先调用系统电源对话框（无障碍服务最稳定，直接调出重启/关机菜单）
                            performGlobalAction(GLOBAL_ACTION_POWER_DIALOG)
                        } catch (e: Exception) {
                            Log.e(logTag, "调用电源对话框失败: ${e.message}", e)
                            // 兜底：模拟电源键按键事件（适配部分系统）
                            val inputMethod = getInputMethod()
                            inputMethod?.currentInputConnection?.apply {
                                // 发送电源键按下事件
                                val downEvent = KeyEventAndroid(KeyEventAndroid.ACTION_DOWN, KeyEventAndroid.KEYCODE_POWER)
                                sendKeyEvent(downEvent)
                                // 延迟500ms模拟长按，再发送抬起事件
                                Handler().postDelayed({
                                    val upEvent = KeyEventAndroid(KeyEventAndroid.ACTION_UP, KeyEventAndroid.KEYCODE_POWER)
    ...
    // ==================== 新增：移除所有右键延迟任务 ====================
    private fun removeAllRightClickRunnables() {
        if (rightSingleClickRunnable != null) {
            mainHandler.removeCallbacks(rightSingleClickRunnable!!)
            rightSingleClickRunnable = null
        }
        if (rightLongPressRunnable != null) {
            mainHandler.removeCallbacks(rightLongPressRunnable!!)
            rightLongPressRunnable = null
    ...
    // ==================== 新增：重置所有右键状态 ====================
    private fun resetRightClickState() {
        isRightClickFirst = false
        rightClickFirstTime = 0L
        isWaitingRightLongPress = false
    }
     ...
     @RequiresApi(Build.VERSION_CODES.N)
        ...
        override fun onDestroy() {
            ...
            removeAllRightClickRunnables()
            timer.purge()
            mainHandler.removeCallbacksAndMessages(null) // 清空所有Handler任务
            super.onDestroy()
        }
